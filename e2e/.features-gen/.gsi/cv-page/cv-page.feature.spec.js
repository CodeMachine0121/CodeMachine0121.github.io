...
test.describe('CV Page (International SWE Style)', () => {
  ...
  test('頁面顯示完整 Resume 內容', { tag: ['@ui'] }, async ({ Given, Then, And, page }) => { 
    ...
    await And('I should see "DevOpsDays Taipei 2026"', null, { page }); 
    await And('the "Export PDF" button should be visible', null, { page }); 
  });

  test('每筆工作經歷顯示 achievements', { tag: ['@ui'] }, async ({ Given, Then, And, page }) => { 
    await Given('I am on the "/cv" page', null, { page }); 
    await Then('I should see "Built cross-regional .NET Core services"', null, { page }); 
    await And('I should see "Migrated the credit loan system"', null, { page }); 
  ...
  test('下載 PDF', { tag: ['@ui'] }, async ({ Given, When, Then, page }) => { 
    await Given('I am on the "/cv" page', null, { page }); 
    await When('I click the "Export PDF" button', null, { page }); 
