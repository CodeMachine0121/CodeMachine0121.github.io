...
Day 02 把 K 線拆成五個數字的時候，留了一句話沒展開：**壓縮是有損的**。今天把那個伏筆收回來，而且不用比喻，直接看資料。

從昨天入庫的 BTC/USDT 現貨資料裡挑 2026-07-15 這一天，找兩根 1 分鐘 K 線：
...
要講公道話：官方 K 線的 12 個欄位裡有 `trade_count` 與 `taker_buy_base_volume`，所以「幾筆」和「主動買佔多少」其實在 1 分鐘的粒度上查得到，Day 02 也已經一併存下來了。逐筆資料真正多給的是**順序**與**分布**：那 80.6% 的主動買是一次吃進來的還是分兩千筆慢慢買的、大單出現在這一分鐘的哪個位置、價格是單調往上還是來回震盪。這些東西沒有任何聚合欄位裝得下，因為它們是路徑，不是總和。
...
## 資料來源：兩條路徑，以及一個要誠實講的限制

Day 03 的原則今天要再用一次：**歷史批次與即時串流是兩條路徑，分開設計。** 只是這一層的兩條路徑不對稱得多。

### 歷史成交走批次，跟 Day 03 同一條路

`data.binance.vision` 有 `aggTrades` 的日檔與月檔，URL 規則跟 K 線那條幾乎一樣，只是路徑裡沒有 timeframe——成交是事件，不是被切好的區間。
...
這裡出現了全系列第一個 `for` 迴圈，所以要交代清楚。「NEVER 用 for loop 遍歷 K 線」這條規矩針對的是**資料量級**：幾十萬列的向量化運算跟逐列迴圈差三個數量級。一筆增量更新裡有十幾檔掛單變動，而每 100 毫秒來一筆，用 DataFrame 表示的話每一筆都要重建一張表，那才是真的跟不上。判準不是「有沒有 for」，是「這個迴圈跑幾次、有沒有向量化的替代品」。
...
class SequenceDecision(StrEnum):
    ...
    - RESYNCHRONIZE：中間漏了幾筆，本地的簿子從這一刻起是錯的。**唯一正確的
      ...
      沒收到的話那一檔會永遠留在本地的簿子上，之後每個特徵都吃到那個幽靈掛單。
    """
...
這是 Day 09 跟前面七天最不一樣的地方。K 線那條路上做錯的決定都可以重跑：欄位對映錯了重新解析一次、清洗規則改了重新跑一次管線，原始的 zip 還在 `data/raw/`。掛單簿沒有這種保險——沒錄下來的東西就是沒有了，而且回不去。
...
也就是說這個特徵**幾乎每秒都在大幅擺動**。這件事對明天很重要：一個每秒換一次臉的特徵，能不能拿來當訊號，是要驗證的，不是假設的。
...
第 4 項是今天真正的重點。前三項是歷史資料對不對，第 4 項是即時路徑的狀態維護對不對，而後者沒有對照組可比——只能靠序號校驗自己說話。那個「0 次」是它唯一的證據。
...
但今天那 227 筆樣本已經先透露了兩件事：這個數字的擺動範圍是 −0.999 到 +0.986，而且幾乎每秒換一次臉。一個這麼躁動的東西到底有沒有預測力，不能靠直覺回答。所以明天除了算出這個特徵，還要做一件本系列到目前為止沒做過的事：**驗證一個特徵值不值得用**——把它跟未來 N 秒的報酬做相關性分析，實際看它有沒有資訊，而不是因為它聽起來合理就收進積木庫。
