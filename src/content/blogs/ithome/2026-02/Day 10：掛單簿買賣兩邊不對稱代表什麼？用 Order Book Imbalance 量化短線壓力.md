...
這裡要誠實補一句：t 值的標準算法假設樣本彼此獨立，而每秒取樣一次的掛單簿顯然不獨立——這一秒的 OBI 跟下一秒的 OBI 高度相關。樣本自相關會讓 t 值偏大，所以 23.78 這個數字要打折看。打多少折沒有簡單的答案（要做 Newey-West 之類的調整），但可以反過來問門檻在哪：要讓它掉到 2 以下，得除以 23.78 / 2 = 11.9，也就是有效樣本數只剩實際的百分之一不到（t 隨樣本數的平方根走，1 / 11.9² ≈ 0.7%）。打對折是 11.9、砍到剩四分之一是 5.9，都還在 2 之上。結論的方向要翻過來，需要的折扣大到不合理。反過來說，如果某天量到一個 t = 2.5 的特徵，在這種取樣結構下就該當成「還不知道」而不是「有」。
...
**取樣密度。** 表頭那 2,508 筆與 45 根，跟開頭錄製回報的數字一模一樣，中間沒有掉東西。平均間隔 1.077 秒也在開頭算過了，而開頭那四列的時間戳差距（1.000、1.006、1.095 秒）都略大於一秒，跟這個平均是同一件事。這也是為什麼後面一律說「往後一筆（約一秒）」而不是「往後一秒」。
