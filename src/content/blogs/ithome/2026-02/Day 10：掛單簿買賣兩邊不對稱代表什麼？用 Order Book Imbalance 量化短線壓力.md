...
|---|---|---|---|---|---|
| 16:52:53 | 3.01816 | 2.10481 | 0.91335 | 5.12297 | +0.178 |
| 16:52:54 | 5.62843 | 0.04242 | 5.58601 | 5.67085 | +0.985 |
| 16:52:55 | 8.28721 | 0.10843 | 8.17878 | 8.39564 | +0.974 |
| 16:52:56 | 3.46778 | 2.43468 | 1.03310 | 5.90246 | +0.175 |
...
聚合這一步還藏著一個容易寫反、而且不會報錯的地方：**要先算比例再平均，還是先把兩側掛量平均起來再算比例。** 這兩件事算出來的是不同的量。用一根 1 分鐘 K 線、每秒一筆共 60 筆、前 30 秒買 1 賣 9、後 30 秒買 90 賣 10 來看，兩條路徑各走一次。

**先算比例再平均（正確）**：前 30 秒每一筆都是 (1 − 9) / (1 + 9) = −0.8，後 30 秒每一筆都是 (90 − 10) / (90 + 10) = +0.8。60 筆取平均，得到 (30 × (−0.8) ＋ 30 × (+0.8)) / 60 = 0.000。
...
為什麼會差這麼多，從中間那個 45.5 就看得出來：它幾乎全部來自後 30 秒的 90（貢獻 45），前 30 秒的 1 只貢獻 0.5。也就是說「先平均掛量」那條路徑把後半段的掛量**絕對量級**當成了權重，前半段那三十筆等於被稀釋掉了；而「先算比例」那條路徑上，兩段各自先被壓成 ±0.8，再以三十比三十的相同權重相加。
...
**第二是 t 值與樣本數。** 相關係數在小樣本上天生就會偏離 0，四十幾個樣本要湊出 0.2 的相關係數，純靠運氣就辦得到。t 值把樣本數納進來，回答「這個相關係數跟 0 分不分得出來」。它只有一條公式：

```
t = IC × √(n − 2) / √(1 − IC²)
```

分子的 √(n − 2) 是樣本數進來的地方——同一個 IC，樣本愈多 t 值愈大。分母的 √(1 − IC²) 讓 IC 愈接近 ±1 時 t 值衝得愈快。慣例是 |t| 小於 2 就當成「跟 0 分不出來」。所以報告裡永遠不能只印一個相關係數。
...
前面三節分別講了未來報酬、秩相關與分組報酬，這一節把它們串成一條路徑實際走一次。用五筆假資料，數字刻意挑得好算。

**第一步，把每一筆快照壓成兩個數字。** 特徵是 OBI（算法就是前面那張四列的表），價格用**中間價**——買一與賣一的算術平均。BTC/USDT 的價差固定 0.01，所以中間價永遠落在半個 tick 上：買一 63,900.00、賣一 63,900.01，中間價是 63,900.005。

**第二步，算未來報酬。** 定義是「（未來的價格 − 現在的價格）／ 現在的價格」，程式裡就是 `prices.shift(-1) / prices - 1.0`。假設這一筆的中間價是 63,900.005、下一筆是 63,900.015，報酬就是 0.01 / 63,900.005 = 1.56 × 10⁻⁷。這個數字小到不好讀，所以報表一律乘以 10,000 換成基點，得到 0.0016 bp。順帶把基點的尺度定下來：**在這個價位上，價格跳一個 tick 就是 0.0016 個基點。**

**第三步，把特徵與未來報酬配成對。** 配好之後長這樣（報酬已換成基點）：
...
**第四步，兩欄各自轉成排名**，由小到大給 1 到 5：

| 時點 | OBI | OBI 排名 | 報酬（bp） | 報酬排名 |
|---|---|---|---|---|
| 1 | +0.90 | 5 | +0.40 | 4 |
| 2 | −0.50 | 2 | −0.30 | 1 |
| 3 | +0.20 | 3 | −0.10 | 3 |
...
**第五步，對這兩欄排名算一般的 Pearson 相關係數**，得到 IC = 0.80。程式裡就是 `feature.rank().corr(forward.rank())` 這一行——`rank()` 是第四步，`corr()` 預設算的就是 Pearson。

沒有同分的時候可以用一條捷徑驗算：ρ = 1 − 6 Σd² / (n(n² − 1))，其中 d 是每一列兩個排名的差。這裡的 d 是 1、1、0、−1、−1，Σd² = 4，代進去是 1 − 24 / (5 × 24) = 0.80，與上面一致。有同分的時候這條捷徑就不成立，而 OBI 有大量的值貼在 ±1，所以程式裡走的是「先排名再 Pearson」那條一般路徑。

**第六步，代 t 值的公式。** 0.80 × √(5 − 2) / √(1 − 0.80²) = 0.80 × 1.732 / 0.600 = 2.31。絕對值勉強超過 2，而樣本只有五筆——這正好是「相關係數在小樣本上天生就會偏離 0」的樣子，也是為什麼報告裡永遠要把 n 印出來。

同一條公式代進今天真正那份資料：IC = 0.4291、n = 2,507，√2,505 = 50.05，√(1 − 0.4291²) = 0.9033，得到 0.4291 × 50.05 / 0.9033 = 23.78。等一下報表上那個 t 值就是這麼來的。

**第七步，分組報酬。** 按 OBI 由低到高排序，切成等量的五組。這裡只有五筆，所以每組剛好一筆（實際資料上每組約 500 筆）：

| 組別（OBI 由低到高） | OBI | 平均報酬（bp） |
|---|---|---|
| 1 | −0.80 | −0.20 |
| 2 | −0.50 | −0.30 |
| 3 | +0.20 | −0.10 |
...
頭尾差是 +0.40 − (−0.20) = 0.60 bp。而這五個數字**不是**單調遞增的（第二組比第一組低，第五組比第四組低），所以報表不會給它「單調」的標記。IC 0.80 這麼高、分組卻不單調，正是前面說的「相關係數是線性摘要，看不出形狀」——兩個數字要一起看的理由就在這裡。
...
讀任何報表的第一步是確認幾個數字對得起來，這裡有三處可以當場對。

**樣本數。** 掛單簿錄到 2,508 筆，往後 1 筆的配對就是 2,508 − 1 = 2,507（最後一筆沒有下一筆，`shift(-1)` 給它 NaN，`dropna()` 把它丟掉）；往後 5 筆是 2,508 − 5 = 2,503，往後 30 筆是 2,508 − 30 = 2,478。K 線那半張表同理：45 根往後 1 根剩 44、往後 3 根剩 42。樣本數對不上，就代表對齊那一步出了事。

**取樣密度。** 45 分鐘理論上該有 2,700 筆每秒取樣，實際是 2,508 筆，平均間隔 45 × 60 / 2,508 = 1.077 秒。這跟開頭那四列的時間戳差距（1.000、1.006、1.095 秒）對得起來——取樣是靠一次一次的 REST 請求，請求本身要花時間，所以間隔會略大於一秒。這也是為什麼後面一律說「往後一筆（約一秒）」而不是「往後一秒」。

**頭尾差。** 它是最右邊五個分組報酬裡「最高組減最低組」，第一列就是 0.16 − (−0.21) = 0.37，對得上。不過這一欄是用未四捨五入的值算的，所以拿表上印出來的兩位小數自己減，偶爾會差 0.01——例如 `obi_10` 往後 1 筆那列，印出來是 0.16 − (−0.22) = 0.38，報表給 0.37，差的 0.01 來自那兩個分組報酬在印出來之前的進位。

對完之後，這張表有四件事可以讀出來，一件一件講。
...
一個基點是萬分之一，也就是 0.01%。所以把一個 bp 的報酬換成錢，就是「價位 × bp ÷ 10,000」。三個數字各換一次：

**收益那一側。** 0.37 bp × 63,900 ÷ 10,000 = **2.36 USDT**——這是最高組與最低組的平均未來報酬之差，也就是「完美地在 OBI 最高的時候做多、最低的時候做空」能抓到的每次價差。換成 tick 是 2.36 / 0.01 = 236 個 tick。注意這個數字已經是最樂觀的版本了：它假設每次都準確地站在分佈的兩端，實際的訊號不會這麼聽話。

**手續費那一側。** Binance 現貨一般用戶的費率是每邊 0.1%。一個基點是 0.01%，所以 0.1% ÷ 0.01% = 10 個基點，進場出場各一次就是 **20 個基點**，在 63,900 上是 20 × 63,900 ÷ 10,000 = **127.8 USDT**。

**價差那一側。** 0.01 ÷ 63,900 = 1.56 × 10⁻⁷，乘以 10,000 得到 0.0016 bp。

| 項目 | 大小（bp） | 在 63,900 上（USDT） |
|---|---|---|
| OBI 前後五分之一組的報酬差（1 秒） | 0.37 | 2.36 |
| 同上（30 秒） | 1.25 | 7.99 |
| 買賣價差來回（0.01 USDT 對 63,900） | 0.0016 | 0.01 |
| 手續費來回（一般費率） | 20 | 127.80 |

價差幾乎可以忽略——BTC/USDT 現貨窄到只有 0.0016 個基點，一次進出的價差成本就是一個 tick。真正的牆是手續費：20 ÷ 0.37 = **54 倍**，就算用 30 秒那個較大的 1.25 個基點，20 ÷ 1.25 也還有 16 倍。
