...
1. **算每一筆取樣的 OBI**——從深度摘要取出前 N 檔的兩側掛量，兩者相減除以相加。兩側都空的那幾筆沒有定義，留白而不是填 0。
2. **把它對齊到 K 線的時間軸**——掛單簿大約一秒一筆、K 線一分鐘一根，所以要壓成一根一個值。壓法是先算好每一筆各自的 OBI，再按分鐘取平均（或取那一分鐘的最後一筆）。順序不能顛倒，理由前面那張 0.000 對 0.654 的表已經說完了。

這兩步要能分開呼叫，因為第二條線只需要第 1 步的結果：驗證要在原始的秒級粒度上做，先聚合再驗，驗到的是「聚合之後還剩多少」，不是這個特徵本身有多少。

**第二條線，判斷它值不值得用：**

3. **算未來報酬**——用中間價，往後 N 筆（聚合之後是 N 根）算一次報酬率，對齊回「現在」這一格，再乘上 10,000 換成基點。同一步順帶把跨越錄製空白的配對作廢。
4. **算秩相關與 t 值**——把特徵與未來報酬各自換成名次、算這兩組名次的相關，再連同樣本數算出 t 值。
5. **算分組報酬**——按 OBI 由低到高切成筆數相同的五組，每組取平均未來報酬，記下頭尾差與是不是單調。
6. **重複跑**——三個深度（前 5／10／20 檔）各配幾個時間跨度，原始取樣與聚合到 1 分鐘各做一遍，排成兩張表。
7. **畫圖**——上格 OBI 疊中間價，下格分組報酬長條圖。

順序上只有一個硬性依賴：第 3 步必須在第 4、5 步之前，而且它是整條流程裡**唯一往未來看的一步**。把它收在一個地方，之後要審「有沒有偷看未來」就只有一個地方要看。
