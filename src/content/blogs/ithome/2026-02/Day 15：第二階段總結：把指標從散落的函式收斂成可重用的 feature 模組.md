...
1. **讀設定檔**——把 YAML 變成一串「一個名字加一組參數」。這一步只管格式對不對（有沒有名字、是不是清單），不管參數的值合不合理。
2. **建出每一個特徵**——照名字查表找到對應的建法，把弱型別的設定值轉成正確的型別。這裡順便擋三種錯：不認識的名字、參數值不合法、以及**參數名拼錯**——沒有被取用的參數一律當成拼錯，直接失敗而不是安靜地用預設值跑完。
3. **問這批特徵需要哪些原料**——K 線、逐筆成交、掛單簿。這件事在算之前就要回答得出來。
4. **只讀真的需要的資料**——一份只有 EMA 與 RSI 的設定不必等那七十幾萬列逐筆成交。原料缺了就在這一步失敗，並指名是哪一個特徵在要它。
5. **逐一算特徵，併成一張表**——所有欄位共用同一條時間索引，所以直接接在一起就好。同名的兩欄會互相蓋掉，所以在建的時候就擋掉重名。
6. **切暖機期**——不是照每個特徵宣告的根數切，因為那個數字只是下限；改成找「第一個所有欄位都有值的位置」。而中間的洞（掛單簿沒錄到的那幾段）要**保留**，那不是暖機期。
7. **落地並印報表**——存成 parquet，並印出每一欄的可用比例與最後一個值。
