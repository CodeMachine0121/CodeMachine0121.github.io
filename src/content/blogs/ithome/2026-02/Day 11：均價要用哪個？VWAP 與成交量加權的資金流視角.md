...
datetime: "2026-09-25"
description: "算術平均把成交 1 顆 BTC 跟成交 100 顆當成一樣重要。這篇先講清楚成交量加權在概念上換掉了什麼（把時間軸換成成交量軸），再實作 VWAP 的兩種累積方式，實測同一根 K 線上兩者相差 164 USDT、而「偏離超過一個標準差」的比例從 28.70% 變成 53.29%；並處理一個看起來只是實作細節的地方——加權變異數照課本那條恆等式直接寫，在 BTC 的價位上會吃掉 float64 一半的有效位數。"
image: ""
...
`float64` 有大約 16 位十進位有效位數。兩個相差八個數量級的數相減，前面那些位數會彼此抵銷掉，答案的有效位數只剩下大約 7 位。這種誤差不會噴例外，也不會產生 NaN——它產生一個看起來很正常、但小數點後幾位是垃圾的數字。而且它的嚴重程度跟標的的價位成正比：同一段程式碼在價格 30 的標的上完全沒事，換到 BTC 就開始掉精度，這讓它特別容易在開發階段被漏掉。
...
平移救回多少，是一件可以測的事，而測法的形狀值得說明。對照組是「直接展開」的定義式 `Σv(p − p̄)² / Σv`，它精度最好但要掃兩遍資料。測試斷言的是：平移過的向量化版本跟它的誤差小於 `1e-9`，而沒平移的版本誤差至少大一百倍。
...
2. **精度那個測試要過**：平移過的加權標準差與定義式 `Σv(p − p̄)² / Σv` 的誤差小於 `1e-9`，而沒平移的版本誤差至少大一百倍。這一項是今天最容易被當成裝飾而跳過的，但它守的是一個不會報錯的錯誤。
