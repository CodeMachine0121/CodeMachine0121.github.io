...
整件事最麻煩的地方在於：**沒有任何錯誤訊息**。`rolling` 不會抱怨，`pandas-ta` 對照組也不會，因為它拿到的是同一張有缺漏的表，兩邊會漂亮地算出同一個錯誤答案。Day 04 到 Day 06 建立的對照組習慣，防的是「公式寫錯」，防不了「資料少了幾列」。
...
步驟 [6] 值得多說一句。它不是額外的檢查，它就是步驟 [1] 再跑一次。管線自己驗自己補完了沒有，比事後開 notebook 查快得多，也不會忘記做。
...
先看一眼今天要動的東西，會發現一件事：**六個步驟裡有四個已經有主人了**。
...
這個檔案很長，而且**它應該是全專案唯一長成這樣的檔案**。所有的 `new` 都集中在這裡，代表其他每一個檔案都不需要知道自己的協作者是誰實作的。
