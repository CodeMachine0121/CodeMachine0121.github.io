...
很多人以為把禁令寫進 rules 就夠了。但只要遇過一次那樣的 `rm -rf`，你就會承認：想擋動作，只有 enforce 層辦得到。
...
一句話記法：不變的界線用 permissions，要看情況的判斷用 hooks。
...
### settings 放哪：個人、專案、本機三層

在動手寫之前，先搞清楚這份 settings 到底放哪個檔，因為放錯層會影響「這道閘是只有你自己踩得到，還是整個團隊都踩得到」。Claude Code 的 settings 分三層，permissions 與 hooks 都寫在這裡：

| 檔案                            | 範圍          | 會不會進 repo             |
|-------------------------------|-------------|-----------------------|
| `~/.claude/settings.json`     | 跨你所有專案的個人設定 | 不會（在家目錄）              |
| `.claude/settings.json`       | 這個專案、跟隊友共用  | 會提交                   |
| `.claude/settings.local.json` | 只在你自己機器上生效  | 不提交（預設進 `.gitignore`） |

三者的優先序是：本機（`settings.local.json`）覆蓋專案，專案覆蓋個人。判斷放哪一層，看這道規則「該由誰遵守」：

- **整個團隊都該一致遵守的守門**——像上面禁改 `internal/db/`、禁碰 `.env`、危險指令要確認——放進 repo 裡的 `.claude/settings.json`。這樣每個 clone 這個專案的人都自動套到同一道閘，不會有人漏設。
- **只屬於你個人的偏好**——例如你自己想多擋幾個指令、或臨時放行某個工具——放 `.claude/settings.local.json`，不會影響到隊友，也不會被提交出去。
- **跨專案都想要的習慣**——例如不管在哪個專案都想讓所有 `rm` 先問過——放家目錄的 `~/.claude/settings.json`。

這篇範例專案的守門規則屬於第一種，所以接下來都寫進 repo 裡的 `.claude/settings.json`。
...
到今天，memory、rules、skills、hooks／permissions 這四塊地基都鋪好了。但這裡藏著一個很多人會有的誤會：把檔案放進 `.claude/` 目錄，不等於 AI 真的讀得到它。目錄裡的東西，哪些會自動進 context、哪些要被檢索或觸發才會被看見，是有邊界的——搞錯了，你會以為某份規則生效了，其實 agent 根本沒讀到。
