 ...
 * 工具函式介面
 */
export interface SeriesUtils {
  /** 建立 URL slug */
  createSlug(seriesName: string, options?: Partial<SlugifyOptions>): string;
  
  /** 排序系列內文章 */
  sortArticles(articles: BlogEntry[], order?: ArticleSortOrder): BlogEntry[];
  ...
  /** 產生麵包屑導覽 */
  createBreadcrumb(seriesName: string, currentPage?: number): BreadcrumbItem[];
  
  /** 驗證 slug 格式 */
  validateSlug(slug: string): boolean;
  
  /** 從文章陣列聚合系列 */
  aggregateSeriesFromBlogs(blogs: BlogEntry[]): Series[];
}
